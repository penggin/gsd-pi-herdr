...
After a registry release, use the guided installer:
...
For pnpm installs, run `pnpm setup` and restart your shell if pnpm says its global bin directory is not in `PATH`.
</Note>
