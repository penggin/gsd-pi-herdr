...
export function checkpointMarker(checkpointId: string): string {
  return [
    `[GSD_CODEX_REMOTE_CHECKPOINT:${checkpointId}]`,
    "Opaque checkpoint injection failed. Do not infer missing history; tell the user to enable",
    "context_management.codex_remote_compaction with the same openai-codex-responses model.",
  ].join(" ");
}
...
export function fallbackSummary(checkpointId: string): string {
  return [
    ...
    "Full replay requires GSD Codex Remote Compaction and the same model through a compatible Codex Responses provider.",
    "Without them, only GSD's retained recent messages remain available.",
  ].join(" ");
}
...
export function latestCheckpoint(entries: readonly SessionEntry[]):
  | { entry: CompactionEntry<CodexCheckpointDetails>; details: CodexCheckpointDetails }
  | undefined {
...
function rawText(item: JsonObject): string {
  ...
  return item.content
    .flatMap((part) => isObject(part) && typeof part.text === "string" && part.type === "input_text" ? [part.text] : [])
    .join("\n");
}
