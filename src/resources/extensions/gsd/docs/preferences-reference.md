...
- `search_provider`: `"brave"`, `"tavily"`, `"ollama"`, `"native"`, or `"auto"` — selects the search backend for research phases. `"native"` forces hosted search on supported Anthropic, direct OpenAI Codex, and explicitly verified Codex-compatible proxy models; provider values force that external backend and disable hosted search; `"auto"` prefers hosted search when the active API/provider pair supports it. Compatible proxies opt in per provider/model with `compat.nativeWebSearch: true`; they remain fail-closed when the field is absent. Default: `"auto"`.
