...
`openai-codex-responses`. Generic `openai-responses`, Azure OpenAI Responses, OpenAI-compatible chat completions,
and external CLI providers continue to use GSD's native plaintext compaction.
...
stores it in `CompactionEntry.details`. Later requests replay it only when the exact model ID, persisted summary
...
- Replay fails closed when a marker is absent or duplicated, retained messages changed, or the model ID differs.
