...
// Private Herdr worker fast-path. This intentionally runs before normal GSD
// startup, extension discovery, onboarding, and provider initialization. The
// worker accepts only a validated owner-only launch.json path generated by GSD.
if (firstArg === '__herdr-worker') {
  const { runHerdrWorkerCli } = await import('./resources/extensions/subagent/execution/herdr-worker/entry.js')
  process.exit(await runHerdrWorkerCli(args))
}
