...
export function inspectHerdrCapabilities({
  ...
  const versionResult = run(binary, ["--version"], { env });
  const version = parseVersion(`${versionResult.stdout}\n${versionResult.stderr}`);
  const schemaText = run(binary, ["api", "schema", "--json"], { env }).stdout;
  const schema = parseJsonFileText(schemaText, "Herdr API schema");
  const methods = collectMethodConstants(schema);
  const paneRunResult = run(binary, ["pane", "run", "--help"], { env });
  const pluginLinkResult = run(binary, ["plugin", "link", "--help"], { env });
