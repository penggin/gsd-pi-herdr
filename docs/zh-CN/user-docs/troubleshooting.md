...
**症状：** 同一个工作单元（例如 `research-slice` 或 `plan-slice`）被反复派发，随后因 artifact 重试耗尽或 liveness wedge 而停止。
...
### 自动模式因 liveness wedge 停止

**症状：** 自动模式报告 wedge id、阻塞它的守卫和获准的恢复操作。再次运行 `/gsd auto` 时，会重复显示同一通知而不会进入循环。

**原因：** 同一守卫或未推进状态的结果两次读取到完全相同的输入。wedge 存储在工作流数据库中，因此其他单元的穿插派发和进程重启都不会清除它。

**解决：** 先执行 wedge 通知中打印的获准恢复操作，再原样运行通知给出的 `/gsd auto --resume-wedge <id>` 命令；该 ID 必须指向当前 open wedge。GSD 会重新检查最初触发 wedge 的守卫，并且只在阻塞解除后恢复。如果恢复操作不完整，wedge 及其计数器会保持不变，自动模式也会继续停止。
...
**解决：** 该问题已在近期版本 修复。如果你仍在旧版本，请更新。现在 dispatch prompt 已包含明确的工作目录指令。
...
**解决：** 已在近期版本 修复。现在 GSD 会自动清理过期的 Web server 进程。如果你还在旧版本，可手动终止孤儿进程：`lsof -ti:3000 | xargs kill`。

### 非 JS 项目被 worktree health check 阻挡

**症状：** 在不使用 Node.js 的项目（例如 Rust、Go、Python）中，worktree health check 失败或阻塞自动模式。

**原因：** 在较早的版本中，worktree health check 只识别 JavaScript 生态。

**解决：** 已在近期版本 修复（#1860）。现在 health check 已支持 17+ 生态。升级到最新版本。
...
**解决：** 已在近期版本 修复。现在所有 git 命令都会强制 `LC_ALL=C`，从而无论系统 locale 如何，都保证 git 输出一致为英文。
...
**解决：** 已在近期版本修复。现在裸 `/gsd` 不会再从运行中的自动模式会话手里抢 session lock。升级到最新版本。

### Worktree 中的提交落到了 main，而不是 `milestone/<MID>` 分支

**症状：** 自动模式在 worktree 中提交时，最终落在了 `main`，而不是 `milestone/<MID>`。

**解决：** 已在近期版本修复。现在 dispatch 前会重新校正 CWD，并在失败时清理过期 merge 状态。升级到最新版本。
...
**解决：** 已在近期版本 修复。现在 extension loader 会自动解析 npm subpath exports，并为动态导入创建 `node_modules` symlink。升级到最新版本。
...
**解决：** 已在近期版本 修复，Windows 现在改用 `where.exe`。升级到最新版本。
...
**原因：** SQLite 数据库未初始化。这个问题会出现在较早的版本中的手动 `/gsd` 会话（非自动模式）中。

**解决：** 已在近期版本 修复。现在数据库会在第一次 tool call 时自动初始化。升级到最新版本。
...
**解决：** 已在近期版本 修复，现在偏好命令会先通过 `isLikelyCommand()` 过滤。请确保偏好中的 `verification_commands` 只包含合法 shell 命令，而不是文字描述。
