...
For GSD Pi application patches and the latest reviewed `open-gsd/gsd-pi` version,
see [GSD Pi upstream review ledger](gsd-upstream.md). This file tracks the separate
vendored Pi engine, not the GSD Pi release line.
...
| Bounded output-limit continuation (R5) | `packages/pi-agent-core/src/{agent-loop,types}.ts`, `packages/pi-agent-core/test/agent-loop-max-tokens.test.ts`, GSD session retry/compaction/event modules | Adapt GSD upstream `5fbf5dca9` without moving downstream next-request preparation before the stop hook. Bound continuation injections, preserve paired tool results and source usage, and make terminal halts explicit. Source-backed session overflow/retry evidence and completion status are tracked in `gsd-upstream-deferred-implementation.md`; this is not a Pi vendor-version update. |
...
| Downstream render oracle maintenance (R6, tests only) | `packages/pi-tui/test/tui-render.test.ts` | Align 13 inherited expectations with the already-existing bottom-anchored short frames, tall-to-tall viewport realignment and dimension-change repaint policy. Assert whole viewport/cursor state, no scrollback clear and actual styled cells; retain semantic reflow/duplicate-history guards. No pi-tui runtime or vendor pin changes. Run the wider source suite with `--experimental-transform-types` for existing TypeScript parameter properties. |
